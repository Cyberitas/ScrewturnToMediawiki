...
function prepareSourceFiles(basedir) {
    ...
    filelist.forEach(function(value, index, list) {
        file = path.join(basedir, value)
        if(path.extname(file) === '.cs')
            pageFileList.push(new ScrewTurnPageFile(file))
    })
...
function convertWiki(pageFiles, destdir) {
    ...
    function processSnapshots() {
        ...
        console.log('Processing ' + lastSnapshot.file.title)
